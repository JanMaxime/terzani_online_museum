...
def search_photos(item, tag_collection, annotation_collection, page_number, page_size):
    ...
    sorted_selected_photos = sorted(selected_photos, key=lambda photo: photo["score"], reverse=True)
    print(len(sorted_selected_photos))
    photo_sorted = [res_dict['images'] for res_dict in sorted_selected_photos if 'images' in res_dict]
    ...
    photo_results = photo_results[page_number*page_size : page_number * page_size + page_size]
    full_images = []
