    ...
    if request.method == "POST":
        ...
        if display_markers:
            ...
            results = search_country(
                request.form["country"], sample_annotations)
            iiifs_and_links = [(json.dumps(result["annotation"]["iiif"]), result["annotation"]["iiif"]["images"]
                                [0]["resource"]["service"]["@id"][:-4] + "/full/,360/0/default.jpg") for result in results]
...
def search_page():
    ...
    if request.method == "POST":
        if request.form["submit"] == "text_search":
            ...
            photos, searched_items = search_photos(
                item, sample_tags, sample_annotations)

            if not display_bb:
                for res_photo in photos:
                    photo = res_photo["annotation"]
                    new_item = (json.dumps(photo["iiif"]), [
                                photo["iiif"]["images"][0]["resource"]["service"]["@id"][:-4] + "/full/,360/0/default.jpg"])
                    ...
                    for sitem in searched_items:
                        ...
                        if box_key:
                            new_item = (json.dumps(photo["iiif"]), [photo["iiif"]["images"][0]["resource"]["service"]["@id"][:-4] + "/" + str(
                                box[0]) + "," + str(box[1]) + "," + str(box[2]) + "," + str(box[3]) + "/max/0/default.jpg" for box in photo["obj_boxes"][box_key]])
                            number_of_results += len(new_item[1])
                            iiif_and_links.append(new_item)
